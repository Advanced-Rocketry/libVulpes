...
import zmaster587.libVulpes.inventory.SlotNoInsertion;
...
public class ModuleSlotArray extends ModuleBase {
	...
	public ModuleSlotArray(int offsetX, int offsetY, IInventory container, int startSlot, int endSlot, boolean canPlayerInsert) {
		super(offsetX, offsetY);
		this.startSlot = startSlot;
		this.endSlot = endSlot;
		this.inventory = container;
	...
	public List<Slot> getSlots(Container container) {
		...
		for(int i = 0; i + startSlot < endSlot; i++) {
			Slot slot;
			slot = canPlayerInsert ? new Slot(inventory, i+startSlot, offsetX + 18* (i % 9), offsetY + 18*(i/9)) : new SlotNoInsertion(inventory, i+startSlot, offsetX + 18* (i % 9), offsetY + 18*(i/9));
